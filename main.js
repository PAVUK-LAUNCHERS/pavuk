...
//   'IDLE'      — DS не запущен нами, лаунчер виден (или может быть скрыт вручную через трей)
...
const state = {
    ...
    community: 'ru'    // 'ru' = русские порты (1984), 'kz' = английские порты (1923)
};
...
// Мягкое уведомление без принудительной блокировки запуска.
...
async function checkForUpdates() {
    ...
    // 1. Основной источник — свой бэкенд
    try {
    ...
    // 2. Fallback — GitHub Releases API (только версия/ссылка на страницу релиза, без прямой ссылки на архив)
    if (!remote) {
        try {
...
ipcMain.on('open-external-url', (event, url) => {
    // Ограничение: открываем только http(s)-ссылки, никаких file://, javascript: и т.п.
...
app.whenReady().then(() => {
    ...
    checkForUpdates(); // первая проверка сразу после старта
